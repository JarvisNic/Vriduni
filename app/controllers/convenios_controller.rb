class ConveniosController < ApplicationController
  ...
  # GET /convenios.json
  def index
      @convenios = Convenio.search(params[:search]).order('nombre ASC').page(params[:page]).per(10)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @convenio.save
        format.html { redirect_to @convenio, notice: 'Su convenio ha sido creado satisfactoriamente. ' }
  ...
  def update
    respond_to do |format|
      if @convenio.update(convenio_params)
        format.html { redirect_to @convenio, notice: 'Su convenio ha sido actualizado satisfactoriamente. ' }
  ...
  def destroy
    @convenio.destroy
    respond_to do |format|
      format.html { redirect_to convenios_url, notice: 'El convenio se ha eliminado correctamente. ' }
  ...
  private
    ...
    def convenio_params
      params.require(:convenio).permit(:nombre, :objetivo, :responsable, :area, :pdf, :categoria, :fecha_inicio, :fecha_fin, :fecha_firma)
    end
end
